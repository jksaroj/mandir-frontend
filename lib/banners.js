...
export async function fetchBanners(placement = "home") {
  ...
  const safePlacement = ["home", "aarti", "temples", "mantra", "chalisa", "blogs", "share_photo"].includes(placement) ? placement : "home";
  const response = await apiGet(`/banners?placement=${encodeURIComponent(safePlacement)}`, { cache: "no-store" });
  const rows = Array.isArray(response?.data) ? response.data : [];
  return rows
    .filter((b) => b?.background_image_url || b?.image_url)
    .map((b) => ({
      id: b.id,
      ...
      slug: b.slug ?? "",
      placement: b.placement || safePlacement,
