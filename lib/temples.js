...
function normalizeTemple(row) {
  ...
  const name = text(row.name, "Temple");
  const description = text(row.description);
  const shortDescription = text(row.short_description ?? row.excerpt ?? row.meta_description);
  const history = text(row.history);
  ...
  const about = row.about ?? {
    paragraphs: [description || shortDescription, history].filter(Boolean),
    info: [
      row.established_year && ["Established", text(row.established_year)],
      row.temple_type      && ["Temple Type", text(row.temple_type)],
      row.festivals_list?.length && ["Festivals", parseList(row.festivals_list).map((f) => text(f.name ?? f)).join(", ")],
      row.temple_authority && ["Temple Authority", text(row.temple_authority)],
      row.annual_visitors  && ["Annual Visitors", text(row.annual_visitors)],
      row.nearest_railway  && ["Nearest Railway Station", text(row.nearest_railway)],
    ].filter(Boolean),
    darshanTimings:
  ...
  let nearby = parseList(row.nearby ?? row.nearby_places).map((item) => {
    ...
    return [
      text(item.name ?? item.title ?? item.place ?? "Nearby place"),
      text(item.distance ?? item.description ?? item.text ?? ""),
    ];
  });
  ...
  const reviews = parseList(row.reviews).map((item) => ({
    ...item,
    reviewer_name: text(item.reviewer_name ?? item.name, "Devotee"),
    review_text: text(item.review_text ?? item.text),
  }));
...
export async function fetchTemples(params = "") {
  ...
  const arr = Array.isArray(response?.data) ? response.data
    ...
    : null;
  if (arr && arr.length > 0) {
    return arr.map(normalizeTemple);
  }
  return temples;
}
