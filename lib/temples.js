...
function normalizeTemple(row, locale = "en") {
  ...
  const text = (value, fallback = "") => {
    ...
    if (typeof value === "object" && !Array.isArray(value)) {
      const localized =
        value[locale] ?? value.en ?? value.hi ?? Object.values(value).find(Boolean);
      return localized == null || localized === value ? fallback : text(localized, fallback);
    }
    return String(value);
  };
  ...
  const looksLikeTiming = (value) => {
    const candidate = cleanText(value);
    return Boolean(
      candidate &&
      candidate.length <= 100 &&
      /\b\d{1,2}(?::\d{2})?\s*(?:AM|PM)\b/i.test(candidate) &&
      !/[.!?।]/.test(candidate)
    );
  };
  const seoDescription = [
    cleanText(row.meta_description),
    cleanText(row.excerpt),
    shortDescription,
    description,
  ].find((value) => value && !looksLikeTiming(value)) || "";
  ...
  return {
    ...
    scheduleTimings,
    excerpt:      seoDescription || `${name} is a sacred Hindu temple${city ? ` in ${city}` : ""}. Explore darshan timings, history, location and visitor information.`,
