...
function normalizeTemple(row) {
  ...
  const about = row.about ?? {
    paragraphs: [row.description, row.history].filter(Boolean),
    info: [
      row.established_year && ["Established", row.established_year],
      row.temple_type      && ["Temple Type", row.temple_type],
      row.temple_authority && ["Temple Authority", row.temple_authority],
      row.annual_visitors  && ["Annual Visitors", row.annual_visitors],
      row.nearest_railway  && ["Nearest Railway Station", row.nearest_railway],
    ].filter(Boolean),
    darshanTimings:
  ...
  if (!Array.isArray(nearby)) {
    try { nearby = JSON.parse(nearby); } catch { nearby = []; }
  }
